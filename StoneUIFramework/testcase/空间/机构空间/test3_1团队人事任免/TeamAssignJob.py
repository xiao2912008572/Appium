...
class TeamAssignJob:
    def __init__(self):  # 初始化测试数据
        ...
        self.AdministratorLoc = int(d.cell("test003-团队", 2, 2))  # 管理员编辑
        self.SalespersonLoc = int(d.cell("test003-团队", 2, 2))  # 销售员编辑
        self.AssistantLoc = int(d.cell("test003-团队", 2, 4))  # 行政助理编辑
        self.AdmNum = int(d.cell("test003-团队", 2, 5))  # 管理员人数
        self.SalNum = int(d.cell("test003-团队", 2, 6))  # 销售员人数
        self.AssNum = int(d.cell("test003-团队", 2, 7))  # 行政助理人数
        self.Name = d.cell("test003-团队", 2, 8)  # 肖静远
        self.Director = int(d.cell("test003-团队", 2, 9))  # 董事会
        self.Marketing = int(d.cell("test003-团队", 2, 10))  # 营销部
        self.HR = int(d.cell("test003-团队", 2, 11))  # 人事部
        # 创建读取配置信息对象
    ...
    def teamAssignJob(self, driver):
        ...
        try:
            ...
            SalNum = int(handle.Kjlb_browseorgspace_menu_team_teamedit_numeidt_jobsnumedit_text())
            self.log.info('当前销售员人数：{0}'.format(SalNum))
            self.log.info('预期销售员人数：{0}'.format(self.SalNum))
            ...
            AssNum = int(handle.Kjlb_browseorgspace_menu_team_teamedit_numeidt_jobsnumedit_text())
            self.log.info('当前行政助理人数：{0}'.format(AssNum))
            self.log.info('预期行政助理人数：{0}'.format(self.AssNum))
            ...
            if driver.find_elements_by_id("com.yunlu6.stone:id/removaljobs_name") != []:  # 列表是否为空
                ...
                for i in range(len(listT)):  # 遍历列表
                    ...
                    if handle.Kjlb_browseorgspace_menu_team_menu_assignjob_contact_text(i) == self.Name:  # 再判断是否该人已被任免
            ...
            self.log.error("TeamAssignJob Inside : %s" % err)
            raise err
