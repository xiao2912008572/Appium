...
class AddPtoPRefuseA(unittest.TestCase):
    ...
    spacename_1 = d.cell("test007-会员", 5, 1)  # 协会测试321
    vipname_1 = d.cell("test007-会员", 5, 2)  # 人脉姓名董祥
    phone1_1 = int(d.cell("test007-会员", 5, 3))  # 账号1:17786174226
    password1_1 = int(d.cell("test007-会员", 5, 4))  # 密码:88888888
    phone2_1 = int(d.cell("test007-会员", 5, 5))  # 账号2:13636059628
    password2_1 = int(d.cell("test007-会员", 5, 6))  # 密码2:88888888
    ...
    def test_addPtoPRefuse(self, spacename, vipname, phone1, password1,
        ...
        :param spacename: 协会名
        :param vipname: 受邀姓名
        :param phone1: 账号：13636059628
        :param password1: 密码：88888888
        :param phone2: 账号：13636059628
        :param password2: 密码：88888888
        :param phone3: 账号：17786174226
        :param password3: 密码：88888888
        :return:
        '''
        try:
            sleep(1)
            ...
            # 4.7退出受邀账号
            self.log.info('退出受邀账号：{0},{1}'.format(phone3, password3))
            self.loginout.loginout(self.driver, 1)
            self.log.info('受邀账号退出完毕')

            # 5.登录管理员账号-检查各处消息
            # 5.1登录
            self.log.info('登录管理员账号：{0},{1} 检查各处消息'.format(phone1, password1))
            self.login.login(self.driver, phone1, password1)
            self.log.info('管理员账号登录完毕')
            sleep(1)
            ...
            # 5.5设置-退出
            self.loginout.loginout(self.driver, 1)
            self.log.info('管理员账号退出完毕')

            # 5.6回到邀请账号
            self.log.info('重新登录邀请账号')
            self.login.login(self.driver, phone2, password2)
            self.log.info('邀请账号登录完毕')
