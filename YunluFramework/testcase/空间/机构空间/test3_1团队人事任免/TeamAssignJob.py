        ...
        try:
            ...
            if driver.find_elements_by_id("com.yunlu6.yunlu:id/removaljobs_name") != []:  # 列表是否为空
                ...
                for i in range(len(listT)):  # 遍历列表
                    ...
                    if handle.Kjlb_browseorgspace_menu_team_menu_assignjob_contact_text(i) == Name:  # 再判断是否该人已被任免
            ...
            # 8.搜索姓名:肖静远
            handle.Kjlb_browseorgspace_menu_team_menu_assignjob_addperson_search_sendkeys(Name)  # 搜索关键字
            ...
            handle.Kjlb_browseorgspace_menu_team_menu_assignjob_contact_click(0)  # 待任免联系人
            self.log.info('点击待任免联系人')
            handle.Kjlb_browseorgspace_menu_team_menu_assignjob_contact_department_click(Director)  # 董事会
            self.log.info('点击董事会')
            ...
            self.log.info('返回')
            name = driver.find_elements_by_id("com.yunlu6.yunlu:id/companyteam_item_name")[0].text  # 获取董事长姓名
